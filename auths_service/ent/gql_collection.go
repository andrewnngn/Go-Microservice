...
func (a *AuthQuery) collectField(ctx context.Context, op *graphql.OperationContext, field graphql.CollectedField, path []string, satisfies ...string) error {
...
func newAuthPaginateArgs(rv map[string]interface{}) *authPaginateArgs {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]interface{}:
			var (
				err1, err2 error
				order      = &AuthOrder{Field: &AuthOrderField{}}
			)
	...
	if v, ok := rv[whereField].(*AuthWhereInput); ok {
		args.opts = append(args.opts, WithAuthFilter(v.Filter))
	}
	return args
}
